...
import { LifeEvents as LifeEventsType } from '~/models'

const Divider = () => {
  return <div className='border border-zinc-100 dark:border-zinc-700/40 w-full my-8' />
}

export default function LifeEvents({
  lifeEvents,
}: {
  lifeEvents: LifeEventsType
}): JSX.Element {
          ...
          <div className='max-w-3xl'>
            ...
            {Object.keys(lifeEvents)
              ...
              .map((year: string, index: number) => (
                <div key={`life-events-${year}-${index}`}>
                  {index !== Object.keys(lifeEvents).length - 1 && index !== 0 && <Divider />}
                  <Event
                    key={`life-events-${year}-${index}`}
                    year={year}
                    events={lifeEvents[year]}
                  />
                </div>
              ))}
